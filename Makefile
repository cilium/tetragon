...
GOLANGCILINT_IMAGE=docker.io/golangci/golangci-lint:v2.11.3@sha256:e838e8ab68aaefe83e2408691510867ade9329c0e0b895a3fb35eb93d1c2a4ba
...
metrics-docs: tetragon-metrics-docs ## Generate metrics reference documentation page.
	...
	echo '{{< /comment >}}' >> $(METRICS_DOCS_PATH)
	$(CONTAINER_ENGINE) run --rm -v $(LOCAL_PWD):$(LOCAL_PWD) -w $(LOCAL_PWD) $(GO_IMAGE) ./tetragon-metrics-docs health >> $(METRICS_DOCS_PATH)
	$(CONTAINER_ENGINE) run --rm -v $(LOCAL_PWD):$(LOCAL_PWD) -w $(LOCAL_PWD) $(GO_IMAGE) ./tetragon-metrics-docs resources >> $(METRICS_DOCS_PATH)
	$(CONTAINER_ENGINE) run --rm -v $(LOCAL_PWD):$(LOCAL_PWD) -w $(LOCAL_PWD) $(GO_IMAGE) ./tetragon-metrics-docs events >> $(METRICS_DOCS_PATH)
