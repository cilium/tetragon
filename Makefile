...
# expand or narrow the scope, e.g.:
#   make test-race RACE_PKGS="./pkg/sensors ./pkg/grpc/..."
...
test-race: tester-progs tetragon-bpf ## Run Go tests with the race detector (use RACE_PKGS=... to override scope).
	$(GO) test -exec "$(SUDO)" -p 1 -parallel 1 $(GOFLAGS) -gcflags=$(GO_BUILD_GCFLAGS) -timeout $(RACE_TEST_TIMEOUT) -race $(RACE_PKGS) ${EXTRA_TESTFLAGS}
