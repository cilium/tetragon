...
static inline __attribute__((always_inline)) __u64
get_ancestor_cgroup_id(const struct cgroup *cgrp, __u64 cgrpfs_ver, __u32 ancestor_level)
{
	__u32 level;
	__u64 id = 0;
	...
	if (level > ancestor_level)
		probe_read(&id, sizeof(id), _(&cgrp->ancestor_ids[ancestor_level]));

	return id;
}

static inline __attribute__((always_inline)) struct cgroup_tracking_value *
__get_cgrp_tracking_val_heap(cgroup_state state, __u32 hierarchy_id, __u32 level)
{
	int zero = 0;
