...
FUNC_INLINE enum caller_filter_result generic_filter_caller(void *ctx, struct msg_generic_kprobe *e, __u32 *f, __u32 seloff)
{
...
#ifdef __LARGE_BPF_PROG
	/* Skip the matchCmdArgs section by reading its length */
	...
	if (e->user_stack_ret == 0) {
		/* Wrapping the get_stack call in with_errmetrics is expensive on older kernels */
	...
	/* Check to appease the verifier, bpf_get_stack() only returns as much entries as fit into the buffer */
