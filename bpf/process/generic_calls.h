...
FUNC_INLINE void do_regs_action(void *ctx, __u32 idx)
{
	__u64 id = get_current_pid_tgid();
	__u32 *idxp;
	 ...
	 * warning in here
	 */
	idxp = map_lookup_elem(&write_offload, &id);
	if (idxp)
		*idxp = idx;
	else
		map_update_elem(&write_offload, &id, &idx, BPF_ANY);
}
#else
#define do_regs_action(ctx, idx)
#endif
