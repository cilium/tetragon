				 ...
				 struct event_config *config, int index)
{
	...
	switch (index) {
	case 0:
		ctx_off = config->off[0];
		asm volatile("%[ctx_off] &= 0xffff;\n"
			     : [ctx_off] "+r"(ctx_off));
		return get_ctx_ul((char *)ctx + ctx_off, config->arg[0]);
	case 1:
		ctx_off = config->off[1];
		asm volatile("%[ctx_off] &= 0xffff;\n"
			     : [ctx_off] "+r"(ctx_off));
		return get_ctx_ul((char *)ctx + ctx_off, config->arg[1]);
	case 2:
		ctx_off = config->off[2];
		asm volatile("%[ctx_off] &= 0xffff;\n"
			     : [ctx_off] "+r"(ctx_off));
		return get_ctx_ul((char *)ctx + ctx_off, config->arg[2]);
	case 3:
		ctx_off = config->off[3];
		asm volatile("%[ctx_off] &= 0xffff;\n"
			     : [ctx_off] "+r"(ctx_off));
		return get_ctx_ul((char *)ctx + ctx_off, config->arg[3]);
	case 4:
		ctx_off = config->off[4];
		asm volatile("%[ctx_off] &= 0xffff;\n"
			     : [ctx_off] "+r"(ctx_off));
		return get_ctx_ul((char *)ctx + ctx_off, config->arg[4]);
	}
	return -1;
}
#endif
