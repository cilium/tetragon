 ...
 */
FUNC_INLINE int
match_cred_uids(__u32 *f, __u32 *index, __u32 flag, __u32 uid)
{
...
{
	__u32 flags, uid, ret = PFILTER_REJECT;
	...
	*index += 4; /* 4: matchCurrentCred flags */
	...
	/* Start handling real UIDs */
	if (flags & PF_CRED_RUID) {
		uid = BPF_CORE_READ(task, cred, uid.val);
		ret = match_cred_uids(f, index, PF_CRED_RUID, uid);
	...
	/* Handle effective UIDs */
	if (flags & PF_CRED_EUID) {
		uid = BPF_CORE_READ(task, cred, euid.val);
		ret = match_cred_uids(f, index, PF_CRED_EUID, uid);
