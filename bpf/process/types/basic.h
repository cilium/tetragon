...
FUNC_INLINE long
filter_arg(struct msg_generic_kprobe *e, struct selector_arg_filter *filter, char *args, int process)
{
	/*
	 * Separate argument filtering based on the process const
	 * for 4.19 kernels..
	 */
	if (process == __FILTER_ARG_1)
		return filter_arg_1(e, filter, args);
	if (process == __FILTER_ARG_2)
		    ...
		    bool is_entry, int process)
{
...
#endif
	{
		...
		if (!is_filter_arg_1(filter->type) && process == __FILTER_ARG_1)
