...
#pragma unroll
	for (i = 0; i < MAX_MATCH_VALUES; i++) {
		__u16 start = v[i * 2], end = v[i * 2 + 1];
		bool res;

		if (is_signed_type(filter->type))
			res = ((__s16)start <= *(s16 *)args) && (*(s16 *)args <= (__s16)end);
		else
...
#pragma unroll
	for (i = 0; i < MAX_MATCH_VALUES; i++) {
		__u8 start = v[i * 2], end = v[i * 2 + 1];
		bool res;

		if (is_signed_type(filter->type))
			res = ((__s8)start <= *(s8 *)args) && (*(s8 *)args <= (__s8)end);
		else
