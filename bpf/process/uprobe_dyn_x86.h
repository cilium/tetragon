...
FUNC_INLINE void revert_ctx(struct pt_regs *ctx, struct pending_call *pc, __u32 tgid, struct task_struct *task)
{
	...
	// (mmap's or dlopen's) has already consumed the original
...
FUNC_INLINE void store_orig_regs(struct pt_regs *ctx, struct pending_call *pc)
{
	...
	pc->orig_addr = ctx->ip;
	// Since we capture expected_sp in the symbol entry,
	// and we'll jump directly to mmap entry,
	// and we will use the expected_sp in the exit retprobes,
	// +8 accounts for `ret` instructions.
	pc->expected_sp = ctx->sp + 8;
}
