...
func PrintTracingPolicies(output io.Writer, policies []*tetragon.TracingPolicyStatus, hookStatus bool, skipPolicy func(pol *tetragon.TracingPolicyStatus) bool) {
	...
	if hookStatus {
		header = "ID\tNAME\tDOMAIN\tSENSOR\tCFGIDX\tHOOKNAME\tHOOKSTATUS"
	}
	...
	for _, pol := range policies {
		...
		for _, sensorInfo := range pol.SensorInfo {
			hook_info := sensorInfo.GetHookInfo()
			if len(hook_info) == 0 {
				fmt.Fprintf(w, "%d\t%s\t%s\t\n", pol.Id, pol.Name, pol.Domain)
				continue
			}

			for i, hi := range hook_info {
				hookStatus := strings.TrimPrefix(strings.ToLower(hi.Status.String()), "status_")
				if i == 0 {
