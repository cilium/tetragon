...
func printFields(w io.Writer, s *apiextensionsv1.JSONSchemaProps) {
	...
	if len(s.Properties) > 0 {
		...
		if s.AdditionalProperties.Schema.Description != "" {
			fmt.Fprintf(w, "%s\n", indent(formatDescription(s.AdditionalProperties.Schema.Description), "     "))
		}
		fmt.Fprintf(w, "\n")
	}
}
	...
	if len(s.Properties) > 0 {
		...
		for _, k := range keys {
			...
			if prop.Description != "" {
				...
				fmt.Fprintf(w, "%s\n", indent(formatDescription(prop.Description), baseIndent))
			}
...
func formatDescription(desc string) string {
	...
	// Return the full description for proper multi-line handling
	return desc
}
