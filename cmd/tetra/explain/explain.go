...
Supported resources:
  tracingpolicy, tp         - TracingPolicy CRD
  tracingpolicynamespaced   - TracingPolicyNamespaced CRD  
  podinfo                   - PodInfo CRD
...
var supportedResources = map[string]resourceInfo{
	"tracingpolicy": {
		...
		aliases: []string{"tp"},
		getCRD: func() *apiextensionsv1.CustomResourceDefinition {
			return &client.TracingPolicyCRD.Definition
		},
	},
	"tracingpolicynamespaced": {
		...
		aliases: []string{"tpns"},
		getCRD: func() *apiextensionsv1.CustomResourceDefinition {
			return &client.TracingPolicyNamespacedCRD.Definition
		},
	},
	"podinfo": {
		kind:    "PodInfo",
		aliases: []string{},
		getCRD: func() *apiextensionsv1.CustomResourceDefinition {
			return &client.PodInfoCRD.Definition
		},
	},
}
...
func listResources(w io.Writer) {
	...
	fmt.Fprintln(tw, "NAME\tSHORTNAMES\tKIND")
	...
	for _, name := range resources {
		info := supportedResources[name]
		shortNames := strings.Join(info.aliases, ",")
		fmt.Fprintf(tw, "%s\t%s\t%s\n", name, shortNames, info.kind)
	}
	tw.Flush()
}
...
func runExplain(w io.Writer, path string) error {
	...
	// Find the resource by name or alias
	var resInfo *resourceInfo

	for resName, info := range supportedResources {
		if resName == resourceName {
			resInfo = &info
			break
		}
		// Check aliases
		if slices.Contains(info.aliases, resourceName) {
			resInfo = &info
			break
		}
	}
	...
	if resInfo == nil {
		...
		for resName, info := range supportedResources {
			supported = append(supported, resName)
			supported = append(supported, info.aliases...)
		}
	...
	crd := resInfo.getCRD()
	if crd == nil {
		return errors.New("failed to load CRD definition")
	}
	...
	fieldPathBuilder.WriteString(resInfo.kind)
	...
	default:
		return outputDefault(w, resInfo.kind, version.Name, parts, current)
	}
}
...
func printFields(w io.Writer, s *apiextensionsv1.JSONSchemaProps) {
	...
	if len(s.Properties) > 0 {
		...
		if s.AdditionalProperties.Schema.Description != "" {
			fmt.Fprintf(w, "     %s\n", formatDescription(s.AdditionalProperties.Schema.Description))
		}
		fmt.Fprintf(w, "\n")
	}
}
...
func printFieldsRecursive(w io.Writer, s *apiextensionsv1.JSONSchemaProps, depth int) {
	...
	if len(s.Properties) > 0 {
		...
		for _, k := range keys {
			...
			fmt.Fprintf(w, "%s   %s\t<%s>%s\n", indent, k, getTypeString(&prop), required)

			if prop.Description != "" {
				fmt.Fprintf(w, "%s     %s\n", indent, formatDescription(prop.Description))
			}
