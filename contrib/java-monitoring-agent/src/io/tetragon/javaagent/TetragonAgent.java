...
/** Instruments entry into Java methods selected by class and method filters. */
public final class TetragonAgent {
  private static volatile JavaIPC ipc;
  ...
  public static void premain(String agentArgs, Instrumentation instrumentation) {
    ...
    try {
      ipc = JavaIPC.open(args.getOrDefault("path", JavaIPC.DEFAULT_PATH));
    } catch (Exception e) {
      System.err.printf("tetragon-java-agent: shared-memory IPC unavailable: %s%n", e);
    }
    instrumentation.addTransformer(new Transformer(className, methodName), false);
    System.err.printf(
        "tetragon-java-agent: monitoring %s#%s via %s%n",
        wildcard(className), wildcard(methodName), args.getOrDefault("path", JavaIPC.DEFAULT_PATH));
  }
  ...
  private static String argument(Map<String, String> args, String key) {
    ...
    if (value == null) {
      throw new IllegalArgumentException(
          "-javaagent arguments must include class=<name>,method=<name>");
    }
    return value;
  }
  ...
  public static void onMethodEntry(
      long methodId, String className, String methodName, String descriptor) {
    JavaIPC producer = ipc;
    if (producer != null) {
      producer.submit(methodId, className, methodName, descriptor);
    }
  }
  ...
  private static final class Visitor extends ClassVisitor {
    ...
    public MethodVisitor visitMethod(
      ...
      if (visitor == null
          || (!targetMethod.isEmpty() && !targetMethod.equals(name))
          || "<clinit>".equals(name)) {
        return visitor;
      }
      ...
      return new AdviceAdapter(Opcodes.ASM9, visitor, access, name, descriptor) {
        ...
        protected void onMethodEnter() {
          ...
          visitMethodInsn(
              INVOKESTATIC,
              "io/tetragon/javaagent/TetragonAgent",
              "onMethodEntry",
