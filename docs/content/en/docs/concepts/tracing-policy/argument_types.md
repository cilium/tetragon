...
`Protocol`, or their negated variants. Address values support CIDR notation.
...
The `syscall64` data type represents a 64-bit syscall argument. This is an
internal type used for automatic argument resolution.

{{< note >}}
The `syscall64` type is used internally by Tetragon. Use the appropriate
specific type (e.g., `int`, `uint64`, `string`) in TracingPolicy definitions.
{{< /note >}}
