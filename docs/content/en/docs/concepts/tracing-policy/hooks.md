...
5.6 or newer is recommended for this feature, and required for
`TracingPolicyNamespaced` (see below).
{{< /note >}}

#### Use in a namespaced policy

`resolvePathInContainer` is the only form of uprobe a
`TracingPolicyNamespaced` may use. A regular uprobe resolves its `path` in the
Tetragon agent's own mount namespace, which is outside the tenant's scope, so a
namespaced policy containing one is rejected at load. With
`resolvePathInContainer` the path resolves under each selected container's root
instead, and the policy only ever selects pods in its own namespace.

A namespaced policy must still set a `podSelector`; use `{}` to select every pod
in the namespace:
...
A namespaced policy is writable by the tenant, so path resolution must stay
inside the container. That containment comes from `openat2` with
`RESOLVE_IN_ROOT`, so on kernels older than 5.6 a namespaced
`resolvePathInContainer` policy is rejected at load rather than resolved with
the uncontained fallback described above. Cluster-scoped policies are written by
a cluster administrator and keep the fallback.
