...
**BEWARE, HERE BE DRAGONS!!! USE WITH CAUTION COULD EASILY CRASH TRACED APPLICATION**
...
The `argRegs` argument is array of strings where each string start with destination
register name followed by `=` and assignment expression, which can be one of the
following types:
...
Note that this interface is likely to be changed in the future.
