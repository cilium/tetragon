...
| tetragon.bpfMapsDisablePrealloc | string | `""` | Comma-separated list of BPF map names to disable preallocation. Only used when bpfMapsPrealloc=true. Supported maps: policy_filter_maps(inner map), override_tasks. |
| tetragon.bpfMapsPrealloc | bool | `true` | Enable preallocation for all supported BPF maps (set to false to disable preallocation). Supported maps: policy_filter_maps(inner map), override_tasks. |
| tetragon.btf | string | `""` |  |
