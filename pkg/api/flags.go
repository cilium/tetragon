...
const (
	...
	// This happens at FGS init when existing processes are being loaded
	// into FGS event buffer. All events should have either EventExecve or
	// EventProcFS set.
	EventProcFS = 0x04
	...
	// parent process in the FGS buffer. This may happen when we did not
	...
	// in the FGS event buffer. If this is set it should be reported to FGS
	// developers for debugging. FGS will do its best to recover information
	...
	EventMiss = 0x40
	// EventNeedsAUID is an *internal flag* for FGS to indicate the audit
	...
	// EventNeedsCWD is an *internal flag* for FGS to indicate the current
	// working directory has not yet been resolved. The FGS hooks look at
