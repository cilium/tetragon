...
func detectLSM() bool {
	...
	_, err := os.Stat("/sys/kernel/security")
	if err != nil {
		if os.IsNotExist(err) {
			if err := os.Mkdir("/sys/kernel/security", 0755); err != nil {
				logger.GetLogger().WithError(err).Error("unable to create /sys/kernel/security directory")
				return false
			}
		} else {
			logger.GetLogger().WithError(err).Error("failed to stat /sys/kernel/security")
			return false
		}
	}
