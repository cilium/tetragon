...
func detectAuditLoginuid() bool {
	enabled, err := kernels.DetectConfig(kernels.CONFIG_AUDIT)
	if err != nil {
		if _, err := os.Stat("/proc/self/loginuid"); err != nil {
			logger.GetLogger().Error("failed to stat /proc/self/loginuid", logfields.Error, err)
			return false
		}
		return true
	}
