...
func (reader *WindowsRingBufReader) Init(fd int, ring_buffer_size int) error {
	...
	if err != nil {
		return fmt.Errorf("Cannot get handle from FD. Error = %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Cannot duplicate handle. Error = %v", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Failed to do device io control. Error = %v", err)
	}
	var buffer uintptr
...
func (reader *WindowsRingBufReader) fetchNextOffsets() error {
	...
	if waitReason != windows.WAIT_OBJECT_0 {
		return fmt.Errorf("Failed in wait function. WaitReason = %d", waitReason)
