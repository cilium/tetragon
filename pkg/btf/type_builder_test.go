...
func TestParseBTFTypeSyntaxErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := ParseBTFType(nil, tt.expr)
			require.Error(t, err)
			if tt.errContains != "" {
...
func TestParseBTFTypePointerArrayCombinations(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ty, err := ParseBTFType(nil, tt.expr)
...
func TestParseBTFTypeFallbackWithKernelSpec(t *testing.T) {
	var kernelSpec *btf.Spec

	btfFiles, err := listBTFFiles()
	fatalOnError(t, err)

	for _, btfFName := range btfFiles {
		kernelSpec, err = btf.LoadSpec(btfFName)
		if kernelSpec != nil {
			break
		}
	}
	...
	if err != nil {
		t.Skip("no BTF file found")
	}

	var taskStruct *btf.Struct
	err = firstTypeByName(kernelSpec, "task_struct", &taskStruct)
	require.NoError(t, err)

	mismatchSpec := specFromBTFTypes(t, []btf.Type{
		&btf.Struct{Name: "other_mismatched_struct", Size: 4},
	})

	tests := []struct {
		name            string
		primarySpec     *btf.Spec
		fallbackErr     error
		wantCalls       int
		...
		{
			name:        "Fallback successfully when primary spec pointer is nil",
			primarySpec: nil,
			wantCalls:   1,
			wantType:    wantPtr(taskStruct),
		},
		{
			name:        "Fallback successfully when primary spec exists but lacks the type",
			primarySpec: mismatchSpec,
			wantCalls:   1,
			wantType:    wantPtr(taskStruct),
		},
		{
			name:            "Bubble up clean error when fallback spec loader fails",
			primarySpec:     nil,
			fallbackErr:     errors.New("vmlinux file path missing or unreadable"),
			wantCalls:       1,
			wantErrContains: "failed to load fallback BTF spec",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			calls := stubFallbackBTFSpec(t, fSpec, tt.fallbackErr)

			ty, err := ParseBTFType(tt.primarySpec, "struct task_struct *")
...
func stubFallbackBTFSpec(t *testing.T, spec *btf.Spec, err error) *int {
	t.Helper()

	orig := loadFallbackBTFSpec
	calls := 0
	loadFallbackBTFSpec = func() (*btf.Spec, error) {
		calls++
		return spec, err
	}
	t.Cleanup(func() {
		loadFallbackBTFSpec = orig
	})
	return &calls
}
