...
// parseCgroupv2RootControllers() parses the controllers from
// /sys/fs/cgroup/cgroup.controllers that indicates active controllers
// that are propagated from root cgroup.
func parseCgroupv2RootControllers() error {
	file := filepath.Join(fmt.Sprintf("%s/1/root/%s", option.Config.ProcFS, cgroupFSPath), "cgroup.controllers")
	...
	}).Debug("Cgroupv2 available controllers")

	fixed := false
	for i, controller := range CgroupControllers {
		if strings.Contains(activeControllers, controller.Name) {
			CgroupControllers[i].Id = 0
			CgroupControllers[i].Active = true
			fixed = true
		}
	}
	...
	if !fixed {
		err = fmt.Errorf("detect cgroup controllers IDs from '%s' failed", file)
		logger.GetLogger().WithFields(logrus.Fields{
			"cgroup.fs": cgroupFSPath,
		}).WithError(err).Warnf("Cgroupv2: controllers 'memory', 'pids' and 'cpuset' are missing")
		// Do not fail just warn and try to continue
		return nil
	}
	...
	for _, controller := range CgroupControllers {
		...
		if controller.Active {
			logger.GetLogger().WithFields(logrus.Fields{
				...
				"cgroup.controller.name":        controller.Name,
				"cgroup.controller.hierarchyID": controller.Id,
			}).Infof("Cgroupv2 supported controller '%s' is active on the system", controller.Name)
		} else {
			// Warn with error
			err = fmt.Errorf("controller '%s' is not active", controller.Name)
			logger.GetLogger().WithField("cgroup.fs", cgroupFSPath).WithError(err).Warnf("Cgroupv2 supported controller '%s' is not active", controller.Name)
		}
	}
...
func DiscoverSubSysIds() error {
	...
	if magic == unix.CGROUP_SUPER_MAGIC {
		...
		/* Parse active cgroup controllers from root cgroup,
		 * this step is helpful for debugging since we may have some
		 ...
		 * or reproduce.
		 */
		return parseCgroupv2RootControllers()
	}
...
func getCgroupv2Controller(cgroupPath string) error {
...
func getValidCgroupv2Path(cgroupPaths []string) (string, error) {
	for _, s := range cgroupPaths {
		if strings.Contains(s, cgroupv2Hierarchy) {
			...
			err = getCgroupv2Controller(cgroupPath)
			if err != nil {
