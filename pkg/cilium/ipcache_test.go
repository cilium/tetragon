...
func TestObserverServer_syncIPCache(t *testing.T) {
	...
	go func() {
		...
		msg := monitorAPI.IPCacheUpsertedMessage("3.3.3.3/32", id100, &id200, nil, nil, 0, "", "")
		n, err := msg.ToJSON()
		require.NoError(t, err)
		ipCacheEvents <- n

		// delete 2.2.2.2
		msg = monitorAPI.IPCacheDeletedMessage("2.2.2.2/32", id100, nil, nil, nil, 0, "", "")
		n, err = msg.ToJSON()
		require.NoError(t, err)
		ipCacheEvents <- n

		// reinsert 2.2.2.2 with pod name
		msg = monitorAPI.IPCacheUpsertedMessage("2.2.2.2/32", id100, nil, nil, nil, 0, "ns-2", "pod-2")
		n, err = msg.ToJSON()
		require.NoError(t, err)
		ipCacheEvents <- n

		// update 1.1.1.1 with pod name
		msg = monitorAPI.IPCacheUpsertedMessage("1.1.1.1/32", id100, &id100, nil, nil, 0, "ns-1", "pod-1")
		n, err = msg.ToJSON()
		require.NoError(t, err)
		ipCacheEvents <- n

		// delete 4.4.4.4
		msg = monitorAPI.IPCacheDeletedMessage("4.4.4.4/32", id100, nil, nil, nil, 0, "", "")
		n, err = msg.ToJSON()
		require.NoError(t, err)
		ipCacheEvents <- n
