...
func pclntabToGosymtab(f *elf.File) (*gosym.Table, error) {
	...
	ptrSize, err := pclntabRejectPreGo126(pclntab, bo)
	if err != nil {
		return nil, err
	}

	if err := pclntabRejectOversizedFunctab(pclntab, bo, ptrSize); err != nil {
		return nil, err
	}
...
// It returns the validated pointer size, so the header prelude is decoded in
// one place.
func pclntabRejectPreGo126(pclntab []byte, bo binary.ByteOrder) (int, error) {
	//  0       4     6   7   8       8+P     8+2P
	// +-------+-----+---+---+-------+-------+-----------+
	...
	if len(pclntab) < 8 {
		return 0, errors.New("pclntab too short")
	}
	ptrSize := int(pclntab[7])
	if ptrSize != 4 && ptrSize != 8 {
		return 0, fmt.Errorf("unexpected ptrSize %d", ptrSize)
	}
	off := 8 + 2*ptrSize
	...
	if textStart != 0 {
		return 0, errors.New("pre-Go 1.26 binary (textStart != 0)")
	}
	return ptrSize, nil
}
...
func pclntabRejectOversizedFunctab(pclntab []byte, bo binary.ByteOrder, ptrSize int) error {
