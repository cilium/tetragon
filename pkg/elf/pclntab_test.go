...
func TestPclntabRejectOversizedFunctab(t *testing.T) {
	...
	if err := pclntabRejectOversizedFunctab(makeHdr(8, 0xFFFFFFFF, 64), binary.LittleEndian, 8); err == nil {
		t.Fatal("expected oversized functab count to be rejected")
	}
	if err := pclntabRejectOversizedFunctab(makeHdr(8, 3, 4096), binary.LittleEndian, 8); err != nil {
		t.Fatalf("expected in-bounds functab count to be accepted: %v", err)
	}
	if err := pclntabRejectOversizedFunctab([]byte{0, 0, 0, 0, 0, 0, 0, 8}, binary.LittleEndian, 8); err == nil {
...
func TestPclntabRejectOversizedFunctabBigEndian(t *testing.T) {
	...
	if err := pclntabRejectOversizedFunctab(hdr, bo, 8); err == nil {
	...
	if err := pclntabRejectOversizedFunctab(hdr, binary.LittleEndian, 8); err != nil {
