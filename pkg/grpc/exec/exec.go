	...
	if useCache {
		if ec := eventcache.Get(); ec != nil &&
			(ec.Needed(tetragonProcess) ||
				...
				(parent != nil && tetragonParent.Pid.Value > 2 && ec.NeededAncestors(ancestors))) {
...
func (msg *MsgCloneEventUnix) HandleMessage() *tetragon.GetEventsResponse {
	...
	if ec := eventcache.Get(); ec != nil {
		...
		if ec.Needed(proc.UnsafeGetProcess()) ||
			msg.MsgCloneEvent.Parent.Pid > 2 && ec.NeededAncestors(ancestors) {
...
func (msg *MsgProcessCleanupEventUnix) HandleMessage() *tetragon.GetEventsResponse {
	...
	if ec := eventcache.Get(); ec != nil &&
		(proc == nil ||
			parent == nil ||
			parent.UnsafeGetProcess().Pid.Value > 2 && ec.NeededAncestors(ancestors)) {
