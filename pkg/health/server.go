...
func StartHealthServer(ctx context.Context, address string, interval int) net.Addr {
	...
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				// Liveness is intentionally pinned to SERVING at startup and never
				// re-evaluated here, so a long initialization window does not
				// cause the kubelet to restart the container. Only the startup
				// status follows GetHealth().
