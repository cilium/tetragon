...
// (see pkg/policyfilter/k8s_test.go). Registration happens during startup, so
// the OnPod* methods return an error to let callers fail initialization rather
// than silently dropping events.
type PodEventSource interface {
	OnPodAdd(handler func(pod *corev1.Pod)) error
