...
func TestPodEventAdapter_OnPodAdd(t *testing.T) {
	...
	var got *corev1.Pod
	require.NoError(t, adapter.OnPodAdd(func(pod *corev1.Pod) {
		got = pod
	}))
...
func TestPodEventAdapter_OnPodAdd_IgnoresWrongType(t *testing.T) {
	...
	called := false
	require.NoError(t, adapter.OnPodAdd(func(_ *corev1.Pod) {
		called = true
	}))
...
func TestPodEventAdapter_OnPodUpdate(t *testing.T) {
	...
	require.NoError(t, adapter.OnPodUpdate(func(oldPod, newPod *corev1.Pod) {
...
func TestPodEventAdapter_OnPodDelete_ConcretePod(t *testing.T) {
	...
	var got *corev1.Pod
	require.NoError(t, adapter.OnPodDelete(func(pod *corev1.Pod) {
		got = pod
	}))
...
func TestPodEventAdapter_RegistersOnePerCall(t *testing.T) {
	...
	require.NoError(t, adapter.OnPodAdd(func(_ *corev1.Pod) {}))
	require.NoError(t, adapter.OnPodUpdate(func(_, _ *corev1.Pod) {}))
	require.NoError(t, adapter.OnPodDelete(func(_ *corev1.Pod) {}))
