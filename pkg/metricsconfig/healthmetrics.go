...
// constraints are only enforced for metrics registered with RegisterWithInit,
...
func registerHealthMetrics(group metrics.Group) {
	...
	// map metrics
	group.MustRegisterWithInit(observer.NewBPFCollector())
	// opcode metrics
	...
	group.MustRegisterWithInit(policystatemetrics.NewPolicyStateCollector())
	// gRPC metrics
