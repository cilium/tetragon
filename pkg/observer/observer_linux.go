...
func (k *Observer) RunEvents(stopCtx context.Context, ready func()) error {
	...
	if useBPFRingBuffer {
		ringBufMapPath = k.RingBufMapPath
	} else {
		ringBufMapPath = k.PerfConfig.MapName
	}

	ringBufMap, err := ebpf.LoadPinnedMap(ringBufMapPath, &pinOpts)
	if err != nil {
		return fmt.Errorf("opening pinned map '%s' failed: %w", ringBufMapPath, err)
	}
