...
const (
	...
	RevMapName = "policy_filter_reverse_maps"
)
...
type PfMap struct {
	dir *ebpf.Map // policy_filter_maps
	rev *ebpf.Map // policy_filter_reverse_maps
}
...
func (m PfMap) release() error {
	if err := releaseMap(m.dir); err != nil {
		return err
	}

	if err := releaseMap(m.rev); err != nil {
		return err
	}
...
func (m polMap) addCgroupIDsReverse(polID PolicyID, cgIDs []CgroupID) error {
	for _, cgID := range cgIDs {
		if err := addReverseMapping(m.Reverse, polID, cgID); err != nil {
			return err
		}
	}
	return nil
...
func addReverseMapping(m *ebpf.Map, polID PolicyID, cgID CgroupID) error {
	...
	name := fmt.Sprintf("policy_reverse_%d_map", cgID)
...
func (m PfMap) newPolicyMap(polID PolicyID, cgIDs []CgroupID) (polMap, error) {
	...
	if err := m.dir.Update(polID, uint32(ret.Inner.FD()), ebpf.UpdateNoExist); err != nil {
		ret.Inner.Close()
	...
	// update reverse map
	for _, cgID := range cgIDs {
		if err := addReverseMapping(m.rev, polID, cgID); err != nil {
			return polMap{}, fmt.Errorf("failed to update reverse map: %w", err)
		}
	}
...
func (m PfMap) deletePolicyIdInReverse(polID PolicyID) error {
	...
	if err := iter.Err(); err != nil {
		return fmt.Errorf("deletePolicyIdInReverse: error iterating reverse map: %w", err)
	}

	// delete empty outer maps
	for _, cgID := range cgIDs {
		if err := m.rev.Delete(cgID); err != nil {
...
type PfMapDump struct {
	Direct  map[PolicyID]map[CgroupID]struct{}
	Reverse map[CgroupID]map[PolicyID]struct{}
}
...
func (m PfMap) readAll() (PfMapDump, error) {
	d, err := readAll[PolicyID, CgroupID](m.dir)
	if err != nil {
...
func (m polMap) delCgroupIDs(polID PolicyID, cgIDs []CgroupID) error {
	...
	for _, cgID := range rmRevCgIDs {
		var id uint32
		if err := m.Reverse.Lookup(cgID, &id); err != nil { // inner map does not exists
			continue
		}
...
func (m PfMap) AddCgroup(polID PolicyID, cgID CgroupID) error {
	...
	if err := m.dir.Lookup(&polID, &innerID); err != nil {
	...
	// reverse map update
	if err := addReverseMapping(m.rev, polID, cgID); err != nil {
		return fmt.Errorf("error updating reverse map: %w", err)
	}
