...
func (m *state) containerExists(pod *podInfo, containerID string, cgIDp *CgroupID) bool {
	for i := range pod.containers {
		...
		m.log.Warn("AddPodContainer: conflicting cgroup ids",
			"pod-id", pod.id,
...
type state struct {
	...
	*logger.DebugLogger
	...
	// policy filters (outer) map handle
	pfMap PfMap
	...
	tracingPolicyTemplates []*tracingPolicyTemplate
}
...
func (m *state) updatePodHandler(pod *v1.Pod) error {
	...
	if err != nil {
		m.log.Warn("policyfilter, pod handler: failed to parse pod id", logfields.Error, err, "pod-id", pod.UID)
		return err
	}
...
func (m *state) addPolicyCommon(policy policy, namespace string, podLabelSelector *slimv1.LabelSelector,
	...
	policy.setFilters(namespace, podSelector, containerSelector)
	...
	for i := range m.pods {
		...
		pod.addCachedPolicy(policy.getID())
	}

	// update state for policy
	if err := policy.AddInitialCgroupIDs(m, cgroupIDs); err != nil {
		for _, pod := range matchedPods {
			pod.delCachedPolicy(policy.getID())
		}
...
// delPolicyCommon will destroy all information for the provided policy
func (m *state) delPolicyCommon(polID PolicyID) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	if policy == nil {
		m.log.Warn("policy internal map not found", "policy-id", polID)
		return nil
	}
	policy.Close(m.log)
	return nil
...
func (m *state) addPodContainers(pod *podInfo, containerIDs []string,
	...
	for i, contID := range containerIDs {
		...
		if m.containerExists(pod, contID, cgIDptr) {
			m.DebugLogWithCallers(4).Info("addPodContainers: container exists, skipping",
				"pod-id", pod.id,
				"namespace", pod.namespace,
				"container-id", contID,
		...
		if cgIDptr == nil {
			...
			if err != nil {
				...
				m.log.Warn("failed to find cgroup id. Skipping container.",
					logfields.Error, err,
					"pod-id", pod.id,
					"container-id", contID)
				continue
	...
	if len(cinfo) == 0 {
		m.DebugLogWithCallers(4).Info("addPodContainers: nothing to do, returning",
			"pod-id", pod.id,
			"namespace", pod.namespace,
			"container-ids", containerIDs)
		return
	...
	m.DebugLogWithCallers(4).Info("addPodContainers: container(s) added",
		"pod-id", pod.id,
		"namespace", pod.namespace,
	...
	for _, policyID := range pod.matchedPolicies {
		...
		if pol == nil {
			m.log.Warn("addPodContainers: unknown policy id found in pod. This should not happen, ignoring.",
				"policy-id", policyID,
				"pod-id", pod.id,
		...
		if err := pol.AddCgroupIDs(m.log, matchingCgIDs); err != nil {
...
func (m *state) delPodCgroupIDsFromPolicyMaps(pod *podInfo, containers []containerInfo) {
	...
	for _, policyID := range pod.matchedPolicies {
		pol := m.findPolicy(policyID)
		if pol == nil {
			m.log.Warn("delPodCgroupIDsFromPolicyMaps: unknown policy id found in pod. This should not happen, ignoring.",
				"policy-id", policyID,
				"pod-id", pod.id)
			continue
		}
		...
		if err := pol.DelCgroupIDs(m.log, cgroupIDs); err != nil {
			// NB: depending on the error, we might want to schedule some retries here
			m.log.Warn("delPodCgroupIDsFromPolicyMaps: failed to delete cgroup ids from policy map",
				logfields.Error, err,
				"policy-id", pol.getID(),
				"pod-id", pod.id,
				"cgroup-ids", cgroupIDs)
...
func (m *state) policiesDiff(pod *podInfo, newLabels labels.Labels) *policiesDiffRes {
	...
	for i := range m.policies {
		pol := m.policies[i]
		podHasPolicy := pod.hasPolicy(pol.getID())
		if pol.podMatches(pod.namespace, newLabels) {
			newMatchedPolicies = append(newMatchedPolicies, pol.getID())
			if !podHasPolicy {
...
func (m *state) applyPodPolicyDiff(pod *podInfo, polDiff *policiesDiffRes) {
	...
	for _, addPol := range polDiff.addedPolicies {
		...
		if err := addPol.AddCgroupIDs(m.log, cgroupIDs); err != nil {
			m.log.Warn(err.Error(),
				"policy-id", addPol.getID(),
				"pod-id", pod.id,
				"cgroup-ids", cgroupIDs,
	...
	for _, delPol := range polDiff.deletedPolicies {
		...
		if err := delPol.DelCgroupIDs(m.log, cgroupIDs); err != nil {
			m.log.Warn("failed to update policy map",
				logfields.Error, err,
				"policy-id", delPol.getID(),
				"pod-id", pod.id,
				"cgroup-ids", cgroupIDs,
...
func (m *state) UpdatePod(podID PodID, namespace, workload, kind string, podLabels labels.Labels,
	...
	dlog := m.DebugLogWithCallers(4).With(
		"pod-id", podID,
		"namespace", namespace,
		"container-ids", containerIDs,
		"container-info", containerInfo,
	...
	if pod.labels.Cmp(podLabels) {
		...
		m.DebugLogWithCallers(1).Info("UpdatePod: pod labels changed",
			"pod-id", pod.id,
