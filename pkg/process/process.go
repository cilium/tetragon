...
func (pi *ProcessInternal) NeededAncestors() bool {
	if option.Config.EnableProcessAncestors && pi.process.Pid.Value > 2 {
		return true
	}
	return false
}
...
// GetAncestorProcesses returns a slice, representing a continuous sequence of ancestors of
// the process up to init process (PID 1) or kthreadd (PID 2), including the immediate parent.
//
// Returns a slice of *tetragon.Process
func GetAncestorProcesses(execId string) ([]*tetragon.Process, error) {
	var ancestors []*tetragon.Process
	...
	for process.process.Pid.Value > 2 {
		...
		ancestors = append(ancestors, process.process)
	}
...
//
// Returns a slice of *ProcessInternal
