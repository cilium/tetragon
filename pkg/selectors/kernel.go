...
func writePrefixBinaries(k *KernelSelectorState, values []string) (uint32, error) {
	return writePrefix(k, values, "MatchBinaries/MatchParentBinaries")
}
...
func writePostfixBinaries(k *KernelSelectorState, values []string) (uint32, error) {
	return writePostfix(k, values, gt.GenericCharBuffer, "MatchBinaries/MatchParentBinaries")
}
...
func ParseGenericMatchBinary(k *KernelSelectorState, b *v1alpha1.GenericBinarySelector, selIdx int, selectorType genericMatchBinariesSelector) error {
	...
	case SelectorOpIn, SelectorOpNotIn:
		for _, s := range b.Values {
			...
			k.WriteGenericMatchBinariesPath(selIdx, s, selectorType)
		}
	case SelectorOpPrefix, SelectorOpNotPrefix:
		if !config.EnableLargeProgs() {
	...
	k.AddGenericMatchBinaries(selIdx, sel, selectorType)

	return nil
}

func ParseGenericMatchBinaries(k *KernelSelectorState, binaries []v1alpha1.GenericBinarySelector, selIdx int, selector genericMatchBinariesSelector) error {
	if len(binaries) > 1 {
		return errors.New("only support a single matchBinaries per selector")
	}
	for _, s := range binaries {
		if err := ParseGenericMatchBinary(k, &s, selIdx, selector); err != nil {
			return err
		}
	}
	return nil
...
func InitKernelSelectorState(args *KernelSelectorArgs) (*KernelSelectorState, error) {
	...
	parse := func(k *KernelSelectorState, selector *v1alpha1.KProbeSelector, selIdx int) error {
		...
		if err := ParseGenericMatchBinaries(k, selector.MatchBinaries, selIdx, matchBinaries); err != nil {
			return fmt.Errorf("parseMatchBinaries error: %w", err)
		}
		if err := ParseGenericMatchBinaries(k, selector.MatchParentBinaries, selIdx, matchParentBinaries); err != nil {
...
func CleanupKernelSelectorState(state *KernelSelectorState) error {
	...
	for selectorID, paths := range state.MatchParentsPaths() {
		sel := state.MatchParents()[selectorID]
		if err := mbset.RemoveID(sel.MBSetID, paths); err != nil {
			errs = errors.Join(errs, err)
		}
	}
