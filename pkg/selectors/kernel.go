...
var actionTypeTable = map[string]uint32{
	...
	"set":                         ActionTypeSet,
	"overrideregs":                ActionTypeOverrideRegs,
}
...
var actionTypeStringTable = map[uint32]string{
	...
	ActionTypeSet:                         "set",
	ActionTypeOverrideRegs:                "overrideregs",
}
...
func HasOverrideRegs(spec *v1alpha1.UProbeSpec) bool {
	for _, s := range spec.Selectors {
		for _, action := range s.MatchActions {
			act := actionTypeTable[strings.ToLower(action.Action)]
			if act == ActionTypeOverrideRegs {
				return true
			}
		}
	}
