...
func TestMultipleSelectorsExample(t *testing.T) {
	...
	expU32Push(4)                 // off: 44      selecotr1: MatchCurrentCred: len
	expU32Push(4)                 // off: 48      selector1: MatchCapabilities: len
...
func TestInitKernelSelectors(t *testing.T) {
	...
	expected_selsize_small := []byte{
		0x1c, 0x01, 0x00, 0x00, // size = pids + args + actions + namespaces + matchCurrentCred + capabilities  + 4
	}

	expected_selsize_large := []byte{
		0x7c, 0x01, 0x00, 0x00, // size = pids + args + actions + namespaces + matchCurrentCred + namespacesChanges + capabilities + capabilityChanges + 4
	}
	...
	if config.EnableLargeProgs() {
		ruid := &v1alpha1.CredentialsSelector{Ruid: ruids}
