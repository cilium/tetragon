...
func TestInitKernelSelectors(t *testing.T) {
	...
	expected_selsize_small := []byte{
		0x40, 0x01, 0x00, 0x00, // size = pids + args + actions + namespaces + capabilities + loginuid  + 4
	}

	expected_selsize_large := []byte{
		0x74, 0x01, 0x00, 0x00, // size = pids + args + actions + namespaces + namespacesChanges + capabilities + capabilityChanges + loginuid + 4
	}
