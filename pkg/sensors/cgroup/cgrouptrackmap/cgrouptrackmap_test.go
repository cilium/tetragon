...
func TestDeepCopyMapValue(t *testing.T) {
	...
	k := &CgrpTrackingValue{
		State:       uint32(ops.CGROUP_RUNNING),
		HierarchyId: 4,
		Level:       5,
	}
	...
	assert.Equal(t, k.State, val.State)
	assert.Equal(t, k.HierarchyId, val.HierarchyId)
	assert.Equal(t, k.HierarchyId, val.HierarchyId)
	assert.Equal(t, bytes.Compare(k.Name[:], val.Name[:]), 0)
