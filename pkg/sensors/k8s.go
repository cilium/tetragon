...
var (
	...
	errNamespacedUprobeBTFPath = errors.New("uprobe btfPath is read from the agent's filesystem and is not supported in a namespaced tracing policy")
...
// uprobes are the exception: they resolve under each selected container's root.
func validateNamespacedProbes(namespace string, spec *v1alpha1.TracingPolicySpec) error {
	...
	for i := range spec.UProbes {
		...
		// btfPath is still opened in the agent's mount namespace, so a tenant
		// could point it at any host file.
		if spec.UProbes[i].BTFPath != "" {
			return errNamespacedUprobeBTFPath
		}
	}
	if len(spec.Usdts) > 0 {
