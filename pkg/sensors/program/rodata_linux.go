...
// registerRodataConfigMapAtInit records the elfName -> pinName mapping for a
// shared rodata config map.
//
// It must only be called at init time. This way there's no locking neded,
// because all package-level var initializers run sequentially.
func registerRodataConfigMapAtInit(elfName, pinName string) {
...
// MapBuilderRodataConfigAtInit creates a shared, read-only, frozen
// configuration map with the given ELF and pin names, populated by load
// func.
//
// load must return a fixed-layout struct usable with encoding/binary.Write
// (fixed-width integer fields and fixed-size arrays thereof only - no
// strings, slices, maps, or pointers). The encoded byte length of the
// returned value must exactly match the compiled size of the corresponding
// ".rodata.<name>" section in the target BPF object, including any C-side
// padding.
//
// It must only be called at init time (e.g. assigned to a package-level
// var, as RodataConfigMap is).
func MapBuilderRodataConfigAtInit(elfName, pinName string, load func() (any, error), lds ...*Program) *Map {
	...
	registerRodataConfigMapAtInit(elfName, pinName)
	return m
}
