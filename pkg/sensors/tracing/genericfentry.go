...
func createGenericFentrySensor(
	...
	for _, fentry := range spec.Fentries {
		if selectors.HasEnforcementAction(fentry.Selectors) {
			return nil, fmt.Errorf("Enforcement is not supported for fentry yet")
		}
	}
...
func (k *observerFentrySensor) LoadProbe(args sensors.LoadProbeArgs) error {
	return loadGenericFentrySensor(args.BPFDir, args.Load, args.Maps, args.Verbose, true)
}

func loadGenericFentrySensor(bpfDir string, load *program.Program, maps []*program.Map, verbose int, fentry bool) error {
