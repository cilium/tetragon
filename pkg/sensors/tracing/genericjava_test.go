...
func javaRecord(t *testing.T) []byte {
	t.Helper()
	msg := javaapi.MsgJava{Common: processapi.MsgCommon{Op: ops.MSG_OP_JAVA, Size: javaapi.MsgJavaSize, Ktime: 123456789}, ProcessKey: processapi.MsgExecveKey{Pid: 100, Ktime: 123456789}, MethodID: 0x1234, TID: 101}
...
func TestHandleJava(t *testing.T) {
	...
	_, events, err := observer.HandlePerfData(javaRecord(t))
	require.Nil(t, err)
	require.Len(t, events, 1)
	msg := events[0].(*grpctracing.MsgJavaEvent)
	ev := msg.HandleMessage().GetProcessJava()
...
func TestHandleJavaRejectsMalformedRecords(t *testing.T) {
	valid := javaRecord(t)
	for _, data := range [][]byte{valid[:len(valid)-1], append(append([]byte{}, valid...), 0)} {
