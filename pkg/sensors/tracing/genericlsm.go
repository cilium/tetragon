...
func addLsm(f *v1alpha1.LsmHookSpec, in *addLsmIn) (id idtable.EntryID, err error) {
	...
	for j, a := range f.Args {
		...
		if a.Resolve != "" && j < api.EventConfigMaxArgs {
			...
			lastBTFType, btfArg, btfpath, err := resolveBTFArg("bpf_lsm_"+f.Hook, &a, false)
			if err != nil {
		...
		argP := argPrinter{index: j, ty: argType, maxData: a.MaxData, label: a.Label, BTFPath: BTFPath}
