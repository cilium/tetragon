...
func createGenericUprobeSensor(
	...
	for _, uprobe := range spec.UProbes {
		...
		hasRegsOverrideAction = hasRegsOverrideAction || selectors.HasOverrideRegs(&uprobe)
	}

	hasSleepableOffload := hasRegsOverrideAction /* && config.EnableV61Progs() */
...
func addUprobe(spec *v1alpha1.UProbeSpec, ids []idtable.EntryID, in *addUprobeIn) ([]idtable.EntryID, error) {
	...
	if selectors.HasOverrideRegs(spec) && !bpf.HasUprobeRegsChange() {
