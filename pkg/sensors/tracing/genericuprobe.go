...
type genericUprobe struct {
	...
	targetPath string
	// linkPath is the attach path: /proc/self/fd/N when linking by fd, or the
	// path resolvePathInContainer resolved inside a container.
	linkPath     string
	symbol       string
	address      uint64
	refCtrOffset uint64
...
type addUprobeIn struct {
	...
	// attachPath overrides the attach/ELF-parse path; events still report
	// spec.Path. Set for resolvePathInContainer.
	attachPath        string
...
func createGenericUprobeSensor(
	...
	for cfgIdx, uprobe := range spec.UProbes {
		// Path lives inside the containers; the reconciler attaches per
		// container instead.
		...
		if len(uprobe.BinaryDigests) != 0 {
			...
			// in.attachPath is the resolved, inode-pinned path, so the digest
			// covers the container's own binary rather than a same-named file
...
// Otherwise it returns attachPath when set, or the path specificed in the urpobe spec.
...
// initUprobeConfig validates and initializes one uprobe. f is nil on the
// validation-only path, which skips the ELF-dependent setup. The returned
// selector state must be attached to uprobe entries or cleaned up.
...
func addUprobe(spec *v1alpha1.UProbeSpec, entryFile *os.File, ids []idtable.EntryID, in *addUprobeIn, has *uprobeHas) (retIDs []idtable.EntryID, retErr error) {
	...
	if entryFile != nil {
		...
		// The in-container Path is not visible in the agent's mount namespace.
