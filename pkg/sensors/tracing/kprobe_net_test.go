...
`
			...
			WithArgs(ec.NewKprobeArgumentListMatcher().
				WithValues(
					...
					ec.NewKprobeArgumentChecker().WithSockArg(
						ec.NewKprobeSockChecker().WithFamily(sm.Full("AF_UNIX")),
					),
				),
			)
	...
	kpAny1 := ec.NewProcessKprobeChecker("uds-any-1").
		WithFunctionName(sm.Full("security_socket_connect")).
		WithArgs(ec.NewKprobeArgumentListMatcher().
			WithValues(
				ec.NewKprobeArgumentChecker().WithSockaddrunArg(
					ec.NewKprobeSockaddrUnChecker().WithFamily(sm.Full("AF_UNIX")),
				),
				ec.NewKprobeArgumentChecker().WithSockArg(
					ec.NewKprobeSockChecker().WithFamily(sm.Full("AF_UNIX")),
				),
	...
	kpAny2 := ec.NewProcessKprobeChecker("uds-any-2").
		WithFunctionName(sm.Full("security_socket_connect")).
		WithArgs(ec.NewKprobeArgumentListMatcher().
			WithValues(
				ec.NewKprobeArgumentChecker().WithSockaddrunArg(
	...
	checker := ec.NewUnorderedEventChecker(kpAny1, kpAny2)
	err := jsonchecker.JsonTestCheckExpectWithKeep(suite.T(), checker, false, false)
	suite.Require().NoError(err)
}
