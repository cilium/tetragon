...
// security_socket_connect still sees the AF_INET6 sockaddr, so an IPv4 CIDR
// must match the IPv4-mapped address through the IPv4 LPM fallback.
...
`
	...
	<-tcpReady
	// Raw syscalls: net.DialTCP("tcp6") rejects IPv4-mapped addresses.
	fd, err := unix.Socket(unix.AF_INET6, unix.SOCK_STREAM, 0)
	suite.Require().NoError(err)
	defer unix.Close(fd)
	err = unix.SetsockoptInt(fd, unix.IPPROTO_IPV6, unix.IPV6_V6ONLY, 0)
	suite.Require().NoError(err)
...
func (suite *KprobeNet) TestKprobeSockaddrUnPathEquals() {
	...
	// Abstract socket names are stored as raw bytes in a 107-byte buffer
	// (UNIX_SOCKET_PATH_MAX-1) padded with nulls. The Equal filter must match
	// all 107 bytes, so pad the name to the full buffer size.
	const absName = "tetragon-equals"
	absPadded := absName + string(make([]byte, 107-len(absName)))

	hook := fmt.Sprintf(`apiVersion: cilium.io/v1alpha1
kind: TracingPolicy
metadata:
...
func (suite *KprobeNet) TestKprobeSockaddrUnFamilyOnly() {
	if !kernels.MinKernelVersion("5.11") {
		suite.T().Skip("skipping test: sockaddr_un tests are gated at kernel >=5.11")
	}
...
kind: TracingPolicy
metadata:
  name: "security-socket-connect-unix-family-only"
spec:
  kprobes:
  - call: "security_socket_connect"
    syscall: false
...
`
	...
	const absSock = "@tetragon-family"
	cleanupAbs := startUnixServer(suite.T(), absSock)
	defer cleanupAbs()
	dialUnix(suite.T(), absSock)

	kp := ec.NewProcessKprobeChecker("uds-family-only-checker").
		WithFunctionName(sm.Full("security_socket_connect")).
		WithArgs(ec.NewKprobeArgumentListMatcher().
			WithValues(
				ec.NewKprobeArgumentChecker().WithSockaddrunArg(
	...
	checker := ec.NewUnorderedEventChecker(kp)
	err := jsonchecker.JsonTestCheckExpectWithKeep(suite.T(), checker, false, false)
	suite.Require().NoError(err)
}
