...
// operator correctly filters events triggered by processes started before
// Tetragon. This is a regression test for GitHub issue #4641 where NotIn
// was not working for processes not in execve_map.
func TestKprobeMatchBinariesEarlyExecNotIn(t *testing.T) {
	testutils.CaptureLog(t, logger.GetLogger())
	ctx, cancel := context.WithTimeout(context.Background(), tus.Conf().CmdWaitTime)
	defer cancel()

	// create temporary files for tail and head commands
	...
	headFile, err := os.CreateTemp("/tmp", fmt.Sprintf("tetragon.%s.head.", t.Name()))
	require.NoError(t, err)
	t.Cleanup(func() {
		headFile.Close()
		os.Remove(headFile.Name())
	})
	...
	// head is NOT in the NotIn list, so its events should be allowed
	headCommand := exec.Command("/usr/bin/head", "-f", headFile.Name())
	err = headCommand.Start()
	require.NoError(t, err)
	defer headCommand.Process.Kill()
	...
	// - Events from /usr/bin/head should be ALLOWED
	matchBinariesTracingPolicy := tracingpolicy.GenericTracingPolicy{
		Metadata: v1.ObjectMeta{
	...
	ops := func() {
		...
		headFile.WriteString("trigger head!")
	}
	events := perfring.RunTestEvents(t, ctx, ops)
	...
	for _, ev := range events {
		if kprobe, ok := ev.(*tracing.MsgGenericKprobeUnix); ok {
			...
			if int(kprobe.Msg.ProcessKey.Pid) == headCommand.Process.Pid {
				headEventExist = true
				continue
			}
	...
	if !headEventExist {
		t.Error("kprobe event triggered by /usr/bin/head (started before Tetragon) should be present, unfiltered by the matchBinaries NotIn selector")
	}
}
