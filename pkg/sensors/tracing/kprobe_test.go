...
func TestLoadKprobeSensor(t *testing.T) {
	...
	if config.EnableLargeProgs() {
		sensorProgs = []tus.SensorProg{
			...
			// retkprobe
			1: {Name: "generic_retkprobe_event", Type: ebpf.Kprobe},
		}
		sensorMaps = []tus.SensorMap{
			// all kprobe programs
			{Name: "process_call_heap", Progs: []uint{0, 1}},

			// kprobe entry and return programs
			{Name: "retprobe_map", Progs: []uint{0, 1}},

			// shared with base sensor
			{Name: "execve_map", Progs: []uint{0, 1}},

			// generic_kprobe_event,generic_retkprobe_event
			{Name: "tcpmon_map", Progs: []uint{0, 1}},

			// generic_kprobe_event,generic_retkprobe_event
			{Name: "socktrack_map", Progs: []uint{0, 1}},
		}

		if config.EnableV511Progs() {
			// generic_kprobe_event,generic_retkprobe_event
			sensorMaps = append(sensorMaps, tus.SensorMap{Name: "tg_conf_map", Progs: []uint{0, 1}})

			// generic_kprobe_event,generic_retkprobe_event
			sensorMaps = append(sensorMaps, tus.SensorMap{Name: "tg_rb_events", Progs: []uint{0, 1}})
		} else {
