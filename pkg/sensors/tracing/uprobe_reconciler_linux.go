...
// onContainerAdd marks the keyed container wanted and attaches it. Informer
// adds come through here.
...
// attachDesired attaches a container an exact reconciliation already committed
// to the wanted set. It never marks the key wanted, so a newer prune cannot be
// undone by this stale off-lock work.
func (r *containerUprobeReconciler) attachDesired(key string) {
	r.mu.Lock()
	admit := r.admitLocked(key)
...
// resolveAndAttach resolves every RIC uprobe path inside the keyed container
// off the lock, then attaches them all-or-nothing under it; any failure skips
// the whole container to be retried later.
...
func (r *containerUprobeReconciler) resolveContainerTargets(key string) ([]resolvedUprobe, func(), bool) {
	...
	for i, t := range r.targets {
		...
		if err != nil {
			// A missing path is expected for sidecar containers lacking the
			// target binary, and an unusable file is container-controlled
			// content; Debug so the resync does not spam Warn.
