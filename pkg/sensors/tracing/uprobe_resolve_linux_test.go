...
func TestResolveBinaryUnderRoot(t *testing.T) {
	...
	t.Run("non-directory root fails closed", func(t *testing.T) {
		...
		require.Error(t, err, "ENOTDIR means something sits at root; must not fall back lexically")
	})
}

// TestResolveBinaryUnderRootWithoutOpenat2 exercises the lexical fallback for
// kernels lacking openat2(RESOLVE_IN_ROOT) via the openat2 seam, so CI covers
// it on any kernel.
func TestResolveBinaryUnderRootWithoutOpenat2(t *testing.T) {
	openat2Orig := openat2
	...
	// Without openat2 the resolver returns the uncontained lexical join.
	got, closeFn, err := resolveBinaryUnderRoot(root, "/usr/bin/app")
	require.NoError(t, err)
	closeFn()
	require.Equal(t, filepath.Join(root, "usr", "bin", "app"), got)

	// The lexical ".." check still runs before the fallback.
	_, _, err = resolveBinaryUnderRoot(root, "/../../etc/shadow")
	require.Error(t, err)

	// The regular-file check also guards the uncontained fallback.
	require.NoError(t, unix.Mkfifo(filepath.Join(root, "usr", "bin", "fifo"), 0o644))
	_, _, err = resolveBinaryUnderRoot(root, "/usr/bin/fifo")
	require.ErrorIs(t, err, errNotRegularFile)
}
