...
var errRICNoPodLister = errors.New("no pod lister supplied for informer snapshots")

// ricPods exposes the current informer cache to snapshot/resync. A func rather
// than pkg/manager's type keeps controller-runtime out of this package. It is
// registered once at startup, before any policy can load.
var ricPods struct {
	sync.RWMutex
...
// src and retains listPods for snapshots. Called once at startup; handlers are
// cheap no-ops until a resolvePathInContainer policy registers a reconciler,
// and the resync loop starts lazily on the first registration.
...
func currentInformerPods() ([]*corev1.Pod, bool) {
	...
	pods, err := listPods()
	if err != nil {
		// Not authoritative: an empty list would prune every attachment.
		logger.GetLogger().Warn("uprobe resolvePathInContainer: failed to list pods, skipping reconcile",
			logfields.Error, err)
		return nil, false
	}
	return pods, true
...
// hooks-only mode derives running containers from informer statuses. The pod
