...
// NewLocalRuner creates a new local runner
//
...
func (r *LocalRunner) RunTest(l *slog.Logger, test *T) *Result {
	...
	if test.ShouldSkip != nil && test.ShouldSkip(r.info) {
	...
	for _, sc := range test.Scenarios {
		...
		// set the scenario timeout to 30s
		// TODO: make it configurable
		ctx, _ := context.WithTimeout(r.cli.Ctx, time.Second*30)
		...
		scenario.Trigger.Trigger(ctx)

		// wait for the checker to return
		checkerRet := <-retChan
		res.ScenariosRes = append(res.ScenariosRes, ScenarioRes{
			Name:       scenario.Name,
