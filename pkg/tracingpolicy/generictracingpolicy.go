...
var (
	TPContext  *CRDContext[*GenericTracingPolicy]
	TPNContext *CRDContext[*GenericTracingPolicyNamespaced]
)

func init() {
	var err error
	TPContext, err = NewCRDContext[*GenericTracingPolicy](&client.TracingPolicyCRD.Definition)
	if err != nil {
		panic(fmt.Sprintf("failed to initialize CRD context for TracingPolicy: %v", err))
	}
	TPNContext, err = NewCRDContext[*GenericTracingPolicyNamespaced](&client.TracingPolicyNamespacedCRD.Definition)
	if err != nil {
