...
type Conf struct {
	...
	PolicyTests string `json:"policytests"`   // policy tests to run: "all" or a path to a file listing test names
}
...
func runPolicyTests(cnf *Conf, doRunTest func(string, string, ...string) (*Result, error)) error {
	...
	var names []string
	if cnf.PolicyTests == PolicyTestAll {
		var err error
		...
		var err error
		names, err = loadPolicyTestNames(cnf.PolicyTests)
		if err != nil {
			return fmt.Errorf("failed to load policy test names from %s: %w", cnf.PolicyTests, err)
		}
	}

	bindir := filepath.Join(cnf.TetragonDir, "contrib", "tester-progs")
	for _, name := range names {
		res, err := doRunTest("policytests."+name, tetraBin,
			"policytest", "run",
...
func waitForTetragon(tetraBin string, timeout time.Duration) error {
	...
	return fmt.Errorf("timeout of %s waiting for tetragon daemon to become ready: last error: %w", timeout, err)
}
